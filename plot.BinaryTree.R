...
node_inner <- function(ctreeobj,
                       ...
                       fill = "white",
                       pval = TRUE,
                       id = TRUE, fontfamily= "DG Meta Science", fontsize=8)
{
  ...
  rval <- function(node, fontfamily= "DG Meta Science", fontsize=8) {
...
node_surv <- function(ctreeobj, fontfamily= "DG Meta Science", fontsize=8, 
                      ylines = 2,
                      id = TRUE, ...)
{
...
node_barplot <- function(ctreeobj,
                         col = "black", fontfamily= "DG Meta Science", fontsize=7,
                         fill = NULL,
                         beside = NULL,
                         ymax = NULL,
                         ylines = NULL,
...
{   
  ...
  rval <- function(node, fontfamily= "DG Meta Science", fontsize=7) {
    ...
    top_vp <- viewport(layout = grid.layout(nrow = 2, ncol = 3,
                       ...
                       width = unit(1, "npc"), 
                       height = unit(1, "npc") - unit(2, "lines"),
                       name = paste("node_barplot", node$nodeID, sep = ""), gp=gpar(fontsize=6))
    ...
    if(beside) {
      ...
      for (i in 1:np) {
        grid.rect(x = xcenter[i], y = 0, height = pred[i], 
                  ...
                  gp = gpar(col = col[i], fill = fill[i],  fontfamily=fontfamily, fontsize=fontsize))
      }
...
node_boxplot <- function(ctreeobj,
                         col = "black",
                         fill = "lightgray"
                         , fontfamily= "DG Meta Science", fontsize=8,
                         width = 0.5,
                         yscale = NULL,
                         ylines = 3,
                         cex = 0.5,
                         id = TRUE)
{
  ...
  rval <- function(node, fontfamily= "DG Meta Science", fontsize=8) {
    ...
    grid.lines(unit(c(xl, xr), "npc"), 
               unit(x$stats[1], "native"), gp = gpar(col = col,  fontfamily=fontfamily, fontsize=fontsize))
    grid.lines(unit(0.5, "npc"), 
               unit(x$stats[1:2], "native"), gp = gpar(col = col, lty = 2,  fontfamily=fontfamily, fontsize=fontsize))
    ...
    grid.lines(unit(c(0.5 - width/2, 0.5+width/2), "npc"), 
               unit(x$stats[3], "native"), gp = gpar(col = col, lwd = 2,  fontfamily=fontfamily, fontsize=fontsize))
    grid.lines(unit(0.5, "npc"), unit(x$stats[4:5], "native"), 
               gp = gpar(col = col, lty = 2,  fontfamily=fontfamily, fontsize=fontsize))
    grid.lines(unit(c(xl, xr), "npc"), unit(x$stats[5], "native"), 
               gp = gpar(col = col,  fontfamily=fontfamily, fontsize=fontsize))
    ...
    if (n > 0) {
      ...
      if (length(index) > 0)
        grid.points(unit(rep.int(0.5, length(index)), "npc"), 
                    ...
                    size = unit(cex, "char"), gp = gpar(col = col,  fontfamily=fontfamily, fontsize=fontsize))
    }
                      ...
                      ...)
{
  ...
  rval <- function(node, fontfamily= "DG Meta Science", fontsize=8) {
    ...
    grid.rect(x = xpos, y = ypos,
              ...
              gp = gpar(col = col, fill = fill,  fontfamily=fontfamily, fontsize=fontsize))
                         ...
                         id = TRUE)
{
  ...
  rval <- function(node, fontfamily= "DG Meta Science", fontsize=8) {
    ...
    if (rug) {
      ...
      grid.lines(x = xr, y = yr, gp = gpar(col = "lightgray",  fontfamily=fontfamily, fontsize=fontsize), 
                 default.units = "native")
      grid.lines(x = xr, y = yr, gp = gpar(col = "lightgray",  fontfamily=fontfamily, fontsize=fontsize), 
                 default.units = "native")
    }
                          ...
                          id = TRUE)
{
  ...
  rval <- function(node, fontfamily= "DG Meta Science", fontsize=8) {
    ...
    grid.rect(gp = gpar(fill = fill[1],  fontfamily=fontfamily, fontsize=fontsize))
    ...
    if (id) {
      nodeIDvp <- viewport(x = unit(0.5, "npc"), y = unit(1, "npc"),
                           ...
                           height = max(unit(1, "lines"), unit(1.3, "strheight", as.character(node$nodeID))))
      pushViewport(nodeIDvp)
      grid.rect(gp = gpar(fill = fill[2], lty = "solid", fontfamily=fontfamily, fontsize=fontsize))
      grid.text(node$nodeID)
      popViewport()
    }
    upViewport()
...
edge_simple <- function(treeobj, digits = 3, abbreviate = FALSE, fontfamily= "DG Meta Science", fontsize=8)
{
  ### panel function for simple edge labelling
  function(split, ordered = FALSE, left = TRUE, fontfamily= "DG Meta Science", fontsize=8) {
...
plotTree <- function(node, xlim, ylim, nx, ny, 
                     ...
                     tnex = 2, drop_terminal = TRUE, debug = FALSE, fontfamily= "DG Meta Science", fontsize=8) {
  ...
  if (node$terminal) {
    ...
    pushViewport(tn_vp)
    if (debug)
      grid.rect(gp = gpar(lty = "dotted", col = 4,  fontfamily=fontfamily, fontsize=fontsize))
    terminal_panel(node) 
    upViewport()
    return(NULL)
  }    
  ...
  plotTree(node$right, c(x0, xlim[2]), c(y1r, 1), nx, ny,
           terminal_panel, inner_panel, edge_panel,
           tnex = tnex, drop_terminal = drop_terminal, debug = debug, fontfamily = fontfamily, fontsize = fontsize)
}
...
plot.BinaryTree <- function(x, main = NULL, type = c("extended", "simple"),
  ...
  ## setup root viewport and device
  ...
  root_vp <- viewport(layout = grid.layout(3, 3, 
                                           ...
                                           widths = unit(c(1, 1, 1), 
                                                         c("lines", "null", "lines"))), 
                      name = "root", gp=gpar(fontfamily=fontfamily, fontsize=fontsize))       
  ...
  if((nx <= 1 & ny <= 1)) {
    ...
    plotTree(ptr,
             ...
             debug = FALSE, fontfamily = fontfamily, fontsize = fontsize)
